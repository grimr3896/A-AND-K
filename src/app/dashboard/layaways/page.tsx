...
import { mockLayaways as initialLayaways, mockProducts } from '@/lib/mock-data';
import type { Layaway, Product } from '@/lib/types';
...
export default function LayawaysPage() {
  ...
  const handleAddLayawaySubmit = (newLayawayData: Omit<Layaway, 'id' | 'lastPaymentDate'>) => {
    const newLayaway: Layaway = {
      ...newLayawayData,
      id: `LAY${(layaways.length + 1).toString().padStart(3, '0')}`,
      lastPaymentDate: new Date().toISOString(),
    };
    setLayaways(prevLayaways => [newLayaway, ...prevLayaways]);
    setIsDialogOpen(false);
    toast({ title: 'Success!', description: `New layaway for ${newLayaway.productName} has been created.` });
  };
  ...
  return (
    <>
      ...
      <AddLayawayDialog
        ...
        onAddLayaway={handleAddLayawaySubmit}
